...
//버퍼쓰기
void DoubleBuffer::WriteBuffer(int _ix, int _iy, char *_string, int _color/*= 흰색*/)
{
	DWORD dw;                           // 좌표 저장 구조체
...
void DoubleBuffer::WriteBuffer(int _ix, int _iy, int _index, int _color /*= 흰색*/)
{
	DWORD dw;                // 좌표 저장 구조체
	COORD CursorPosition;    // 좌표 설정
	...
	WriteFile(m_HBuffer[m_iBufferIndex], pstr, strlen(pstr), &dw, NULL);
}
