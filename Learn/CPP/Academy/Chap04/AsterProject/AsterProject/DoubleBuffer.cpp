...
void CDoubleBuffer::ClearBuffer()
{
	...
	FillConsoleOutputCharacter(m_HBuffer[m_iBufferIndex], ' ', 120 * 80, Coor, &dw);
}

// 버퍼 해제
void CDoubleBuffer::DestroyBuffer()
{
	SafeCloseHandle(m_HBuffer[0]);    // 1번 버퍼 해제
	SafeCloseHandle(m_HBuffer[1]);    // 2번 버퍼 해제
									  // (HANDLE이 지정한 정수값은 그대로. 해당 정수에 대한 HANDLE 커널만 삭제)

									  //::SAFE_DELETE(m_Inst);
}
// 안전하게 CloseHandle을 처리하기 위해 MyCloseHandle 만듬
bool CDoubleBuffer::SafeCloseHandle(HANDLE& hFile)
{
